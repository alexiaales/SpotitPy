...
ps = IntVar()
T = Text(root, height = 5, width = 52)  
l = Label(root, text = "Define the persentile: ",bg = '#142841',fg='#AFB9BF')
l.config(font =("arial bold", 10))
l.place(x=70,y=470)
l = Label(root, text = "       ",bg = "#C3CBCE",fg='#152635',width=4)
l.config(font =("arial bold", 10))
...
# Wavelet-based background and noise subtraction for fluorescence microscopy images
    ...
    noise_lvl = int(noise_l[0]) #default = 1
...
def tracking(img1,img2,number_of_cells,mask_cyto,image,img):
    ...
    f2 = tp.locate(img2,int(particle_size),percentile=int((persentil)[0]))
    plt.figure(figsize=(50,50))
    ...
    for cells in range(len(np.unique(mask_cyto))):
        if cells!=0:
            ...
            frame = pd.Series(np.zeros(len(f1['y'])))
            f1=f1.assign(frame=frame.values)
            f2 = tp.locate(removed_mask2,int(particle_size),percentile=int((persentil)[0]))
            frame = pd.Series(np.ones(len(f2['y'])))
            f2=f2.assign(frame=frame.values)        
...
def test1():
    s=var_s.get()
    var_s = IntVar()
c1=Checkbutton(root, text="1",variable=var_s,onvalue = 1, offvalue = 0,bg='#142841',command = test1).place(x=130, y=510)
c2=Checkbutton(root, text="2",variable=var_s,onvalue = 2, offvalue = 0,bg='#142841',command = test1).place(x=178, y=510)
c3=Checkbutton(root, text="3",variable=var_s,onvalue = 3, offvalue = 0,bg='#142841',command = test1).place(x=223, y=510)
c4=Checkbutton(root, text="4",variable=var_s,onvalue = 4, offvalue = 0,bg='#142841',command = test1).place(x=268, y=510)
c5=Checkbutton(root, text="5",variable=var_s,onvalue = 5, offvalue = 0,bg='#142841',command = test1).place(x=313, y=510)
c6=Checkbutton(root, text="6",variable=var_s,onvalue = 6, offvalue = 0,bg='#142841',command = test1).place(x=359, y=510)
c7=Checkbutton(root, text="7",variable=var_s,onvalue = 7, offvalue = 0,bg='#142841',command = test1).place(x=410, y=510)
